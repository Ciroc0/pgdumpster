...
describe("backup resume partial cleanup", () => {
  ...
  it("does not clean completed steps during resume", async () => {
    const { workspaceRoot, checkpointPath } = await fixture();
    const registry = await loadCoverageRegistry();
    const runId = randomUUID();
    const completedPath = path.join(workspaceRoot, "complete.bin");
    await writeFile(completedPath, "complete\n");

    const initialStep: BackupStep = {
      id: "all",
      consistency: {
        snapshot: () => Promise.resolve({ revision: 1 }),
        cleanup: () => Promise.resolve(),
    ...
    const rerun = vi.fn(() => Promise.reject(new Error("must not rerun")));
    const resumedStep: BackupStep = {
      id: "all",
      run: rerun,
      consistency: {
        snapshot: () => Promise.resolve({ revision: 1 }),
    ...
    await expect(
      ...
      manifest: { result: { consistency: "verified" } },
    });
