...
| A.3–A.4                  | implemented + evidenced          | Preceding candidate SHA `f336302e20df2802303d498671b9aa8cc8dcafd2` passed CI run `31978134499`, including Node 22/24 on Ubuntu, macOS and Windows. The final `0.1.2` candidate must repeat this evidence.                                                                                                                                                                                                                              |
...
| P.185                    | implemented + evidenced          | CodeQL run `31978134451` completed successfully for preceding candidate SHA `f336302e20df2802303d498671b9aa8cc8dcafd2`; the final `0.1.2` candidate must repeat the workflow.                                                                                                                                                                                                                                                          |
| P.186                    | release-time/external gate       | The first tag stopped before publication because npm SBOM could not inspect the pnpm workspace layout. The fixed workflow generates the SBOM from a fresh production install of the packed artifact; the final candidate must execute that tagged flow.                                                                                                                                                                                |
...
| Q.201–Q.203              | implemented but evidence missing | Preceding candidate `f336302e20df2802303d498671b9aa8cc8dcafd2` passed protected Live E2E run `31978451505` with terminal 55-component coverage, 20 verified planned restore actions, matching database/direct Storage/Auth smokes and restored Edge Function invocation. The final `0.1.2` candidate must repeat it; Vault ciphertext, Edge secret plaintext and private Auth signing material remain explicit manual/platform limits. |
...
The immediate local implementation order is complete. Official contract drift, protected hosted E2E and tagged release actions remain release-time/external gates.
