...
| H.93                     | platform/manual limit            | The deployed representation and restore handler are covered locally, but the current Edge secret endpoint exposes digests rather than source plaintext. Since functions depend on those secrets, deployment to target requires a documented manual secret-substitution procedure before target inventory/parity can be claimed.                                                                                                                                    |
