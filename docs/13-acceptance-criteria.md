...
| A.3–A.4                  | implemented + evidenced          | Preceding candidate SHA `761fdb16864528ae641c82c8b08bc33139a40c4c` passed CI run `31971524896`, including Node 22/24 on Ubuntu, macOS and Windows. The final candidate must repeat this evidence.                                                                                                                                                                                                   |
...
| P.185                    | implemented + evidenced          | CodeQL run `31971524917` completed successfully for preceding candidate SHA `761fdb16864528ae641c82c8b08bc33139a40c4c`; the public code-scanning endpoint reported zero open alerts. The final candidate must repeat the workflow.                                                                                                                                                                  |
| P.186                    | release-time/external gate       | SBOM is generated by the implemented tagged workflow, which must not run before all exact-SHA release evidence and npm bootstrap are complete.                                                                                                                                                                                                                                                      |
...
| Q.201–Q.203              | implemented but evidence missing | The current candidate has a managed encrypted source-to-clean-target run with terminal 55-component coverage, 20 verified planned restore actions, matching database/direct Storage/Auth smokes and restored Edge Function invocation. Protected workflow execution remains; Vault ciphertext, Edge secret plaintext and private Auth signing material remain explicit manual/platform limits.      |
