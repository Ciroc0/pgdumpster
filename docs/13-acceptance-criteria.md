...
The checkboxes below are the binding normative requirements, not a release-progress UI. The evidence table is the current status ledger. **Implemented + evidenced** means current local tests and/or recorded live evidence; **implemented but evidence missing** means the code exists but needs proof; **implementation missing** means a required path is absent; **platform/manual limit** means automatic exact fidelity is not available and requires an operator procedure; **release-time/external gate** requires GitHub/npm/repository action.
...
| A.7–A.8                  | implemented + evidenced       | Public `v0.1.2` installation and package audit passed; the PolyForm Shield license, notices, contribution, community and security files are present and aligned with the source-available model.                                                                                                                                                                                                    |
...
| K.117–K.127              | implemented + explicit limits | Capture/classification is covered. Every mutable control-plane path without a current documented write contract remains an explicit fail-closed manual/platform limit.                                                                                                                                                                                                                              |
...
If automation cannot access credentials/resources required for the live E2E, it must state that the implementation is awaiting that external release gate. It must not replace the evidence with a claim that mocked/local tests are equivalent.
