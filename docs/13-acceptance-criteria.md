...
| Q.194                    | implemented but evidence missing | Deterministic 10k small-object orchestration, 100k inventory, 32 MiB object-stream and 64 MiB database-dump regressions exist. A second 128 MiB Cloudflare R2 baseline measured 13.61 MiB/s, 34 requests, zero observed retries, 154,140,672-byte peak RSS and checkpoint-state persistence; comparative provider fault/retry checkpoint-overhead evidence remains. |
...
The immediate local implementation order is therefore: preserve the managed E2E evidence, then close candidate package/manual-limit evidence. Protected GitHub workflow, CI/CodeQL and release actions remain external gates.
