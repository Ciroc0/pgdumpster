...
| H.88–H.92                | implemented but evidence missing | The invalid Management API body path remains rejected. A guarded Supabase CLI `functions download --use-api` source-tree capture plus isolated `functions deploy --use-api` handler has local regression coverage; a current managed source-to-target function deploy/invocation proof remains required.                                                                                                                                                           |
