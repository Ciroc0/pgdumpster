...
- `.tar.zst.age` input for inspect/coverage/verify and restore through configured `encryption.identityFile`;
...
- **Encrypted input**: `.tar.zst.age` is supported by inspect/coverage/verify and both restore modes when config supplies `encryption.identityFile`. pgDumpster never needs the private key value as a CLI argument.
...
- **Live recovery evidence**: the normal local live source-to-clean-target execution and the protected GitHub Environment E2E for release SHA `dd0d42c128907de473f71024196a62da2f124bcb` completed with backup `complete_with_platform_limits`, offline verification `verified`, restore/parity `restored_with_platform_limits`, 55 terminal coverage components, 20 verified planned restore actions, matching database, direct stream-hashed private Storage, restored Auth password login and restored Edge Function invocation smokes. The local run is evidence for ordinary operator execution. The GitHub run is separate exact-SHA release evidence and uses CI-specific credentials, networking and runner setup; it neither replaces local recovery evidence nor proves every GitHub runner can connect to every project. Supabase returns worker-local `file:///tmp/...` metadata after deploy; this is normalized as non-portable while the CLI-downloaded source tree remains the deploy authority. PgBouncer, Auth secret/signing and Edge-secret-digest remain manual/platform limits. A future release candidate must repeat the protected run. Cloudflare R2 S3 interoperability passed.
