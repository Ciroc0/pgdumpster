...
- **Hosted E2E**: a disposable-source encrypted backup, offline verify, clean-target restore and narrow database semantic-parity observation completed as `restored_with_platform_limits`. It verified account/job counts, enum, RLS, trigger/checksum behavior and Realtime publication after resume recovery. The source had no File Storage objects and no S3-compatible destination, so Storage/provider interoperability and the final fully applicable E2E remain pending.
