...
Release SHA `dd0d42c128907de473f71024196a62da2f124bcb` has successful exact-SHA GitHub CI, CodeQL, contract-drift and protected E2E runs on 2026-08-16. CI run `31979435085`, CodeQL run `31979435100`, contract-drift run `31979441181` and E2E run `31979442398` completed successfully before the successful `v0.1.2` release workflow run `31979898052`.
...
- [x] Implement Auth config/SSO/TPA/signing metadata, modern/legacy API keys, Edge Function/secret capture plus guarded CLI source-tree deploy, Vault root-key capture and broad control-plane adapters.
...
- [x] Execute the fixed SBOM/provenance/package-smoke/release workflow for `v0.1.2` (run `31979898052`). `v0.1.0` failed before `npm publish` because npm SBOM could not read the pnpm workspace dependency tree; `v0.1.1` reached npm but its GitHub token had no npm registry authorization. Neither version is published.
- [x] Run protected hosted E2E for release candidate `dd0d42c128907de473f71024196a62da2f124bcb` (run `31979442398`). Local managed evidence includes database/File Storage/Auth/control-plane plus Edge Function source-tree deploy/invocation; Vault ciphertext, Edge secret plaintext, and private Auth signing material remain documented manual/platform limits.
...
- At this historical checkpoint, the fixture had no File Storage bucket/object and no external S3-compatible destination. The later File Storage, Cloudflare R2 and protected E2E observations below supersede those evidence gaps.
